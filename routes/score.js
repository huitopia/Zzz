...
router.get('/scores/:userIdx', authMiddleware, async (req, res) => {
  const { userIdx } = req.params;
  const { user } = res.locals;
  try {
    const arrIdx = [Number(userIdx)]; // Diary schema에 DB 존재하는지 파악
    ...
    for (let z = 0; z < userIdxDbobj.userIdxDb.length; z++) {
      arrIdxDb.push(Number(userIdxDbobj.userIdxDb[z].userIdx));
    }
    ...
    if (tokenUser === dbUser) {
      ...
      // {monthDiary: [{ yearMonth: '2022-1', day: 1, sleepScore: 4 }]}
